...
pull_source "https://github.com/popcornmix/xbmc/archive/newclock5.tar.gz" "$(pwd)/src"
...
export LD_LIBRARY_PATH=/usr/lib/jvm/java-7-openjdk-armhf/jre/lib/arm/jli:$LD_LIBRARY_PATH
if [ "$1" == "rbp1" ] || [ "$1" == "rbp2" ] || [ "$1" == "vero" ] || [ "$1" == "vero2" ] || [ "$1" == "vero3" ]
then
    BUILD_OPTS=$(($BUILD_OPTS + $BUILD_OPTION_NEEDS_SWAP))
fi
build_in_env "${1}" $(pwd) "mediacenter-next-osmc-noaddon" "$BUILD_OPTS"
build_return=$?
if [ $build_return == 99 ]
then
